...
//    }
    ...
    @GetMapping("/cars/datas/dto/{id}")
    public ResponseEntity<Object> getCarsByIdDTO(@PathVariable("id") long id) throws Exception {
        Cars cars = carsService.findByIdCars(id);

        if (cars != null) {
            CarsDTO carsDTO = modelMapper.map(cars,CarsDTO.class);
            return new ResponseHandler().
                    generateResponse(ConstantMessage.SUCCESS_FIND_BY, HttpStatus.OK,carsDTO,null,null);
    ...
    @GetMapping("/cars/dto/like/{carType}")
    public ResponseEntity<Object> findCarsDTO(@PathVariable("carType") String carType) throws Exception {

        int data = 0;
        List<Cars> lsCars = carsService.findCarTypeContain(carType);
...
//    @GetMapping("/cars/dto/like/{carType}")
//    public ResponseEntity<Object> getCarsLikeDTO(@PathVariable("carType") String carType)throws Exception{
//
//        Cars cars= (Cars) carsService.findCarTypeLikes(carType);
//        CarsDTO carsDTO=modelMapper.map(cars, CarsDTO.class);
//        return new ResponseHandler().
//                generateResponse(ConstantMessage.SUCCESS_FIND_BY,HttpStatus.OK,carsDTO,null,null);
//    }
    ...
    public ResponseEntity<Object>
    saveAllCars(@Valid @RequestBody List<Cars> listCars
    ) throws Exception {
        carsService.saveAllCars(listCars);
        return new ResponseHandler().generateResponse(ConstantMessage.SUCCESS_SAVE,HttpStatus.CREATED,null,null,null);
    }

    @PostMapping("/insrt/cars/cat/{id}")
    public void addCategory(@RequestBody Category category,@PathVariable("id") Long carsId) throws Exception {
        carsService.addCategory(category, carsId);
    }
