...
public class CarsService {
    ...
    public void saveCars(Cars cars) throws Exception{
        if (cars.getCarNum()==null)throw new DataIntegrityViolationException(ConstantMessage.WARNING_CARNUM_NAME_MANDATORY);
        if (cars.getCarType()==null)throw new DataIntegrityViolationException(ConstantMessage.WARNING_CARTYPE_MANDATORY);
        if (cars.getCarBrand()==null)throw new DataIntegrityViolationException(ConstantMessage.WARNING_CARBRAND_MANDATORY);
        if (cars.getCarColor()==null)throw new DataIntegrityViolationException(ConstantMessage.WARNING_CARCOLOR_MANDATORY);
        if (cars.getCarYear()==null)throw new DataIntegrityViolationException(ConstantMessage.WARNING_CARYEAR_MANDATORY);
    ...
    public List<Cars> findCarTypeContain(String carType)
    {
        return carsRepo.findByCarTypeContaining(carType);
    }

    @Transactional(rollbackFor = {Exception.class})
    public void saveAllCars(List<Cars> ls)
    {
        carsRepo.saveAll(ls);
    }

    public void addCategory(Category category, Long carsId) throws Exception {
        Cars cars = carsRepo.findById(carsId).
                orElseThrow(() -> new ResourceNotFoundException(ConstantMessage.WARNING_CAR_NOT_FOUND));
        cars.getCategory().add(category);
        saveCars(cars);
    }
